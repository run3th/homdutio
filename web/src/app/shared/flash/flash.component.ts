...
/**
 * The transient overlay body rendered by {@link FlashService} — a plain single-line confirmation (e.g.
 * assignment feedback). Owns its own auto-dismiss timer (cleared on destroy so a manual dismiss never
 * double-fires) and calls back to dispose the host overlay.
 */
@Component({
  selector: 'app-flash',
  imports: [],
