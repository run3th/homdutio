...
- **Live target** (`deploy-plan.md`): App Service `homdutio`, RG `homdutio-rg`, plan `homdutio-plan` (B1 Linux, Poland Central), HTTPS-only, `SCM_DO_BUILD_DURING_DEPLOYMENT=false`, subscription `<SUBSCRIPTION_ID>`. **No deployment slots on B1** → rollback = redeploy the prior artifact by hand.
