...
- [ ] 2.2 forgot-password returns identical 200 for known and unknown email
- [ ] 2.3 Known email triggers the fake sender, capturing a non-empty link/token
- [ ] 2.4 reset-password with captured token + valid new password returns 200 and new password logs in
- [ ] 2.5 reset-password with invalid/expired token returns a generic failure (no enumeration)
- [ ] 2.6 reset-password with a weak new password returns `ValidationProblem`
- [ ] 2.7 After a successful reset, a previously issued refresh token is rejected
- [ ] 2.8 forgot-password is rate-limited (429 after threshold)

#### Manual

- [ ] 2.9 End-to-end against LocalDB + real ACS: request → email → reset → log in
- [ ] 2.10 A reset link older than 1 hour is rejected with the generic failure
