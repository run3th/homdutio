...
| ---------- | ----- | ----- | ------ | --------- |
| DEC-1 | [#1](https://github.com/run3th/homdutio/issues/1)  | [DEC-1] Live-update transport: polling vs SignalR | `type:decision` `status:blocked` | — |
| DEC-2 | [#2](https://github.com/run3th/homdutio/issues/2)  | [DEC-2] Transactional email provider for password reset | `type:decision` `status:proposed` | — |
...
| F-02  | [#4](https://github.com/run3th/homdutio/issues/4)  | [F-02] Mount ASP.NET Identity + cookie-auth pipeline | `type:foundation` `status:proposed` `stream:a` | v1 MVP |
| F-03  | [#5](https://github.com/run3th/homdutio/issues/5)  | [F-03] Establish 5s live-update transport (polling vs SignalR) | `type:foundation` `status:blocked` `stream:b` | v1 MVP |
...
| S-06  | [#12](https://github.com/run3th/homdutio/issues/12) | [S-06] Single-use invite, join, live shared board | `type:slice` `status:blocked` `stream:b` | v1 MVP |
...
- F-02 → #3 (F-01)
- F-03 → blocked on #1 (DEC-1)
- S-01 → #3 (F-01), #4 (F-02)
- S-02 → #7 (S-01)
- S-03 → #8 (S-02)
- S-04 → #9 (S-03)
- S-05 → #9 (S-03)
- S-06 → #8 (S-02), #5 (F-03); blocked on #1 (DEC-1)
- S-07 → #9 (S-03)
- S-08 → #7 (S-01); see #2 (DEC-2)
- S-09 → #12 (S-06)
- DEC-1 blocks #5 (F-03), #12 (S-06)
- DEC-2 informs #14 (S-08), non-blocking
