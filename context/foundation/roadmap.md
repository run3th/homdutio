...
2. **Transactional email provider for password reset (FR-020).** ✅ RESOLVED (2026-05-30): **SendGrid.** Initially set to Azure Communication Services Email for co-location, but ACS requires a verified sender *domain*; SendGrid's single-sender verification (one from-address, no domain ownership) and ~100/day free tier fit reset-only v1 better. Trade: a third-party account + API key (store in App Service settings / Key Vault). Affects: S-08. ↩️ **REVERSED (2026-06-25): Azure Communication Services Email.** The original concern (ACS needs a verified *domain*) is mitigated by ACS's **Azure-managed domain** — a free `*.azurecomm.net` sender subdomain with no DNS/domain ownership. ACS wins on first-party Azure co-location, no third-party account, and **managed-identity (Entra ID) auth — no connection string or key anywhere** (the App Service managed identity is granted access to the ACS resource; the app reads only the non-secret `AcsEmail:Endpoint` + `AcsEmail:SenderAddress`). Trade: managed-domain sends may land in spam until a custom domain is verified (deferred). Implemented in S-08 Phase 1 via `Azure.Communication.Email` + `Azure.Identity` (`DefaultAzureCredential`).
